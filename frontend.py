...
def get_selected_row(event):
    ...
    # selected tuple is the row we want
        e1.delete(0, END)
...
def search_command():
    ...
    for row in backend.search(title_text.get(), author_text.get(), year_text.get(), isbn_text.get()):
        list1.insert(END, row)
# .get() as StringVar to string conversion needed
# title_text and etc are the text inside the entry boxes
...
# [0] to fetch the ID from the tuple of data as ID is first data value
...
# bind method links function to a widget event
...
b3 = Button(window, text="Insert entry", width=15, command=insert_command)
